...
func startListening() {
	...
	serverMuxA.HandleFunc("/", apiHandler)
	...
	go func() {
		...
		log.Print("Beginning to serve on port :8000")
		log.Fatal(http.ListenAndServe("localhost:8000", serverMuxA))
	}()
	// Prometheus metrics endpoint
	log.Print("Beginning to serve on port :8080")
	log.Fatal(http.ListenAndServe("localhost:8080", serverMuxB))
}
